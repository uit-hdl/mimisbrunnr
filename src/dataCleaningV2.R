...
source(paste0(MAIN_PROJECT_FOLDER,"src/lib/tools/toolsBasic.R"),    encoding="utf-8")
source(paste0(MAIN_PROJECT_FOLDER,"src/lib/tools/toolsLatex.R"),    encoding="utf-8")
source(paste0(MAIN_PROJECT_FOLDER,"src/lib/tools/toolsPlotting.R"), encoding="utf-8")
...
#      a copypaste from all the sources.
{
  ...
  # Also, delete the garbage column that we need to create the dataframes     
  {
    ...
    # --------------------------------
    {
        ...
        pubertyWomenTable$Breats         = originalBiomarkersTable$BREASTS_FEMALE_FF1
...
# ---- Contraceptives
{
    ...
    # Diseases
    {
        ...
        # Anyway, lets create the relational table for diseases
        {
            ...
            # Count how many diseases we have
            {
                ...
                for(i in 1:originalTotalRows){
                        ...
                        if(!is.na(diseaseTable$Rhinitis[i])) if(diseaseTable$Rhinitis[i] == 1 ){
                            ...
                            diseasesDBDF[currentCell,1] = diseaseTable$ID[i]
                            diseasesDBDF[currentCell,2] = "Rhinitis"
                        ...
                        if(!is.na(diseaseTable$Asthma[i])) if(diseaseTable$Asthma[i] == 1 ){
                            ...
                            diseasesDBDF[currentCell,3] = "J45" # J45 Generic Asthma, there are more subcodes if you want to specify
                            diseasesDBDF[currentCell,4] = "Respiratory system"
            ...
            {
                diseasesDBDF[diseasesDBDF$Diagnostic == "Høysnuve (allergisk rinitt)",]$Diagnostic       = "Rhinitis"
            ...
            # -- Change all the "Others" text to something that makes sense (There is about 145, and you need to look for the ICD10 too T_T)
            {
                ...
                # ------------------------------------------
                diseasesDBDF[diseasesDBDF$Diagnostic == "epilepsi",]$ICD10       = "G40.909"
                diseasesDBDF[diseasesDBDF$Diagnostic == "epilepsi",]$Diagnostic  = "Epilepsy, unspecified, not intractable, without status epilepticus"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Epilepsi-juvenil" ,]$ICD10       = "G40.B19"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Epilepsi-juvenil" ,]$Diagnostic  = "Juvenile myoclonic epilepsy, intractable, without status epilepticus"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Epilepsi",]$ICD10       = "G40.909"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Epilepsi",]$Diagnostic  = "Epilepsy, unspecified, not intractable, without status epilepticus"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Epilepsi\nEpilepsi og Blødersykdom Won Willibr",]$ICD10       = "G40.909"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Epilepsi\nEpilepsi og Blødersykdom Won Willibr",]$Diagnostic  = "Epilepsy, unspecified, not intractable, without status epilepticus"
                newRow = c(20048317, "Von Willebrand's disease", "D68.0", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                diseasesDBDF[diseasesDBDF$Diagnostic == "epilepsi\nepilepsi  fra 6 år og daglige magesmerter fra 10 år.",]$ICD10       = "G40.909"
                diseasesDBDF[diseasesDBDF$Diagnostic == "epilepsi\nepilepsi  fra 6 år og daglige magesmerter fra 10 år.",]$Diagnostic  = "Epilepsy, unspecified, not intractable, without status epilepticus"
                newRow = c(20098322, "Unspecified abdominal pain", "R10.9", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                diseasesDBDF[diseasesDBDF$Diagnostic == "Prob med finmotorikken + utredet",]$ICD10       = "G96.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Prob med finmotorikken + utredet",]$Diagnostic  = "Disorder of central nervous system, unspecified"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Charcot-Marie-Tooth type 1A",]$ICD10       = "G60.0"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Charcot-Marie-Tooth type 1A",]$Diagnostic  = "Hereditary motor and sensory neuropathy"        
                diseasesDBDF[diseasesDBDF$Diagnostic == "cerebral parese"  ,]$ICD10     = "G80.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "cerebral parese",]$Diagnostic  = "Cerebral palsy, unspecified"
        
                # This got translated but no ICD10 code was added, also adding unspecified 
                diseasesDBDF[diseasesDBDF$Diagnostic == "Migraine",]$ICD10       = "G43.909"        
                diseasesDBDF[diseasesDBDF$Diagnostic == "Migraine",]$Diagnostic  = "Migraine, unspecified"      
        
                # ------------------------------------------
                # Psychological / Psychiatric
                # ------------------------------------------
                diseasesDBDF[diseasesDBDF$Diagnostic == "løpsk tale",]$ICD10                      = "F98.6"
                diseasesDBDF[diseasesDBDF$Diagnostic == "løpsk tale",]$Diagnostic                 = "Cluttering"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Selvskading. Kutting.",]$ICD10           = "X78"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Selvskading. Kutting.",]$Diagnostic      = "Intentional self-harm by sharp object"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Diagnostisert med ME (CFS)",]$ICD10      = "R53.82"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Diagnostisert med ME (CFS)",]$Diagnostic = "Chronic Fatigue Syndrome"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Asbergers",]$ICD10                       = "F84.5"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Asbergers",]$Diagnostic                  = "Asperger's syndrome"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Asberger syndrom",]$ICD10                = "F84.5"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Asberger syndrom",]$Diagnostic           = "Asperger's syndrome"
                diseasesDBDF[diseasesDBDF$Diagnostic == "atypisk autisme (diagnose)",]$ICD10      = "F84.0"
                diseasesDBDF[diseasesDBDF$Diagnostic == "atypisk autisme (diagnose)",]$Diagnostic = "Autistic disorder"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Psykose" ,]$ICD10                        = "F29"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Psykose" ,]$Diagnostic                   = "Unspecified psychosis"
        
                # ------------------------------------------
                # Heart and blood
                # ------------------------------------------        
                diseasesDBDF[diseasesDBDF$Diagnostic == "Mangler aortaklaffen, plages med tungpust og smerter i brystet. Medfødt.",]$ICD10       = "Q25.4"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Mangler aortaklaffen, plages med tungpust og smerter i brystet. Medfødt.",]$Diagnostic  = "Other congenital malformations of aorta"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Født med AV-blokk. Innlagt pacemaker." ,]$ICD10      = "I44.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Født med AV-blokk. Innlagt pacemaker." ,]$Diagnostic = "Atrioventricular block, complete"
                diseasesDBDF[diseasesDBDF$Diagnostic == "artrius lesoria? ( blodåre som går rundt spiserør/ pusterør)",]$ICD10       = "Q27.8"
                diseasesDBDF[diseasesDBDF$Diagnostic == "artrius lesoria? ( blodåre som går rundt spiserør/ pusterør)",]$Diagnostic  = "Other specified congenital malformation of peripheral vascular system"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Bærer av blodsykdom. Mangler Von Willebrandt faktor.",]$ICD10       = "D68.0"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Bærer av blodsykdom. Mangler Von Willebrandt faktor.",]$Diagnostic  = "Von Willebrand's disease"        
                # ---- Low iron, so I'm guessing anemia
                diseasesDBDF[diseasesDBDF$Diagnostic == "tar jerntilskudd etter at prøve viser lave jernlagre (3-4 måneder siden diagnose)",]$ICD10       = "D64.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "tar jerntilskudd etter at prøve viser lave jernlagre (3-4 måneder siden diagnose)",]$Diagnostic  = "Anemia, unspecified"
        
                # ------------------------------------------
                # Lungs
                # ------------------------------------------
                # ---- Asthma cases
                diseasesDBDF[diseasesDBDF$Diagnostic == "Sportsastma?",]$ICD10                      = "J45"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Sportsastma?",]$Diagnostic                 = "Asthma"                
                diseasesDBDF[diseasesDBDF$Diagnostic == "Anstrengelsesutløst astmasymptomer etter mycoplasmainfeksjon 2 år siden",]$ICD10       = "J45" 
                diseasesDBDF[diseasesDBDF$Diagnostic == "Anstrengelsesutløst astmasymptomer etter mycoplasmainfeksjon 2 år siden",]$Diagnostic  = "Asthma"
                # ---- For whatever reason, we classified all animal allergies as simple rhinitis
                diseasesDBDF[diseasesDBDF$Diagnostic == "insektsallergi",]$ICD10                      = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "insektsallergi",]$Diagnostic                 = "Rhinitis"        
                diseasesDBDF[diseasesDBDF$Diagnostic == "en del allergi, mest mot dyr",]$ICD10        = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "en del allergi, mest mot dyr",]$Diagnostic   = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergisk mot pelsdyr",]$ICD10               = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergisk mot pelsdyr",]$Diagnostic          = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergisk mot pelsdyr",]$ICD10               = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergisk mot pelsdyr",]$Diagnostic          = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergi mot pelsdyr",]$ICD10                 = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergi mot pelsdyr",]$Diagnostic            = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi hund og kattt og midd",]$ICD10       = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi hund og kattt og midd",]$Diagnostic  = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi mot katt",]$ICD10                    = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi mot katt",]$Diagnostic               = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi mot katt og hund",]$ICD10            = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi mot katt og hund",]$Diagnostic       = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergisk husdyr",]$ICD10                    = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergisk husdyr",]$Diagnostic               = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergisk mot hund og katt",]$ICD10          = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergisk mot hund og katt",]$Diagnostic     = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergisk mot hund, katt og timotei.",]$ICD10      = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergisk mot hund, katt og timotei.",]$Diagnostic = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi for pollen, pelsdyr og melkeproteiner og noe hun ikke husker.",]$ICD10       = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi for pollen, pelsdyr og melkeproteiner og noe hun ikke husker.",]$Diagnostic  = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "pollenallergi\npollenallergi og allergi mot gulrot og epler, hund katt og hest",]$ICD10      = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "pollenallergi\npollenallergi og allergi mot gulrot og epler, hund katt og hest",]$Diagnostic = "Rhinitis"
        
                # ---- Added the rhinitis plus the plant allergy
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi mot pelsdyr og timotei" ,]$ICD10      = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "allergi mot pelsdyr og timotei" ,]$Diagnostic = "Rhinitis"
                newRow = c(20069219, "Allergic contact dermatitis due to plants, except food", "L23.7", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
        
                # ---- Assuming respiratory allergy since nothing else is indicated
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergi",]$ICD10                         = "J30.81"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Allergi",]$Diagnostic                    = "Rhinitis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Pollenallergi",]$ICD10                   = "J30.1"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Pollenallergi",]$Diagnostic              = "Allergic rhinitis due to pollen"
                diseasesDBDF[diseasesDBDF$Diagnostic == "pollenallergi hver sommer",]$ICD10       = "J30.1"
                diseasesDBDF[diseasesDBDF$Diagnostic == "pollenallergi hver sommer",]$Diagnostic  = "Allergic rhinitis due to pollen"        
        
                # ------------------------------------------
                # Bones diseases and Physical traumas
                # ------------------------------------------
                # ---- Other disorders
                diseasesDBDF[diseasesDBDF$Diagnostic == "hypermobile ledd",]$ICD10                                          = "M35.7"
                diseasesDBDF[diseasesDBDF$Diagnostic == "hypermobile ledd",]$Diagnostic                                     = "Hypermobility syndrome"
                diseasesDBDF[diseasesDBDF$Diagnostic == "senebetennelse i venstre legg siste to år. Tørr hud",]$ICD10       = "M65.20"
                diseasesDBDF[diseasesDBDF$Diagnostic == "senebetennelse i venstre legg siste to år. Tørr hud",]$Diagnostic  = "Calcific tendinitis"
                newRow = c(20012409, "Xerosis cutis", "L85.3", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                diseasesDBDF[diseasesDBDF$Diagnostic == "Barneleddgikt",]$ICD10                                             = "M08"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Barneleddgikt",]$Diagnostic                                        = "Juvenile arthritis"
               # ---- Rheumatism covers arthritis, so nothing else added
                diseasesDBDF[diseasesDBDF$Diagnostic == "Reumatisme. Juvenil ideopatisk artritt." ,]$ICD10                  = "M08"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Reumatisme. Juvenil ideopatisk artritt." ,]$Diagnostic             = "Juvenile arthritis"
        
                # -------- Only Arthritis, possible diagnostics are left out
                diseasesDBDF[diseasesDBDF$Diagnostic == "hypermobile ledd og mulig bechtrew, leddgikt",]$ICD10              = "M13"
                diseasesDBDF[diseasesDBDF$Diagnostic == "hypermobile ledd og mulig bechtrew, leddgikt",]$Diagnostic         = "Other arthritis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Dystrofia Myotonica",]$ICD10                                       = "G71.11"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Dystrofia Myotonica",]$Diagnostic                                  = "Myotonic muscular dystrophy"
                # ---- Head
                diseasesDBDF[diseasesDBDF$Diagnostic == "Myalgisk encfalopati",]$ICD10       = "M79.1"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Myalgisk encfalopati",]$Diagnostic  = "Myalgia"
                # ---- Neck
                diseasesDBDF[diseasesDBDF$Diagnostic == "Nakkeplager. Fått fysikalsk behandling. I bedring",]$ICD10       = "M54.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Nakkeplager. Fått fysikalsk behandling. I bedring",]$Diagnostic  = "Cervicalgia"
                diseasesDBDF[diseasesDBDF$Diagnostic == "nakkesmerter",]$ICD10                                            = "M54.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "nakkesmerter",]$Diagnostic                                       = "Cervicalgia"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Nakkesmerter på høyre side etter skade" ,]$ICD10                 = "M54.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Nakkesmerter på høyre side etter skade" ,]$Diagnostic            = "Cervicalgia"
        
                # ---- Spine and back
                diseasesDBDF[diseasesDBDF$Diagnostic == "Skoliose",]$ICD10      = "M41"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Skoliose",]$Diagnostic = "Scoliosis"
                diseasesDBDF[diseasesDBDF$Diagnostic == "skoliose",]$ICD10      = "M41"
                diseasesDBDF[diseasesDBDF$Diagnostic == "skoliose",]$Diagnostic = "Scoliosis"
                # -------- Adding back pain as independent from scoliosis
                diseasesDBDF[diseasesDBDF$Diagnostic == "Scoliose. Brukt korsett i 1 år 2009.Har ryggsmerter.",]$ICD10                = "M41"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Scoliose. Brukt korsett i 1 år 2009.Har ryggsmerter.",]$Diagnostic           = "Scoliosis"
                newRow = c(20053919, "Dorsalgia, unspecified", "M54.9", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                # -------- Adding, knee, back, and shoulder pain as indepedent from scoliosis
                diseasesDBDF[diseasesDBDF$Diagnostic == "kne,rygg og skulderskade etter en ulykke.  Skoleriose i rygg",]$ICD10       = "M41"
                diseasesDBDF[diseasesDBDF$Diagnostic == "kne,rygg og skulderskade etter en ulykke.  Skoleriose i rygg",]$Diagnostic  = "Scoliosis"
                newRow = c(20091214, "Dorsalgia, unspecified", "M54.9", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                newRow = c(20091214, "Unspecified superficial injury of knee",     "S80.91", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                newRow = c(20091214, "Unspecified superficial injury of shoulder", "S40.91", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                diseasesDBDF[diseasesDBDF$Diagnostic == "Beinmargsødem. Prolaps i korsryggen.",]$ICD10      = "M51.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Beinmargsødem. Prolaps i korsryggen.",]$Diagnostic = "Other specified intervertebral disc displacement"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Bechtrew",]$ICD10       = "M45.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Bechtrew",]$Diagnostic  = "Ankylosing spondylitis of unspecified sites in spine"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kroniske ryggsmerter etter en idrettskade. Går nå med nakkekrage etter en nakkeskade",]$ICD10       = "M54.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kroniske ryggsmerter etter en idrettskade. Går nå med nakkekrage etter en nakkeskade",]$Diagnostic  = "Dorsalgia, unspecified"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Smerter i korsryggen i minst 4 år",]$ICD10          = "M54.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Smerter i korsryggen i minst 4 år",]$Diagnostic     = "Dorsalgia, unspecified"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Plaget med vondt i ryggen i flere år",]$ICD10       = "M54.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Plaget med vondt i ryggen i flere år",]$Diagnostic  = "Dorsalgia, unspecified"
                ...
                diseasesDBDF[diseasesDBDF$Diagnostic == "Plages med ryggen siden den ene foten er lengre enn den andre",]$ICD10       = "M54.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Plages med ryggen siden den ene foten er lengre enn den andre",]$Diagnostic  = "Dorsalgia, unspecified"
        
                # ---- Hips
                diseasesDBDF[diseasesDBDF$Diagnostic == "CLP (hofteskade)",]$ICD10       = "M91.12"
                diseasesDBDF[diseasesDBDF$Diagnostic == "CLP (hofteskade)",]$Diagnostic  = "Legg–Calvé–Perthes disease (LCPD)"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Hoftefeil begge som er medfødt",]$ICD10       = "M24.15"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Hoftefeil begge som er medfødt",]$Diagnostic  = "Other articular cartilage disorders, hip"
                # ---- Legs
                diseasesDBDF[diseasesDBDF$Diagnostic == "Plaget med rygg og knær etter fall i skibakke.",]$ICD10      = "S89.90"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Plaget med rygg og knær etter fall i skibakke.",]$Diagnostic = "Unspecified injury of unspecified lower leg"
                diseasesDBDF[diseasesDBDF$Diagnostic == "skade i venstre kne" ,]$ICD10      = "S89.82"
                diseasesDBDF[diseasesDBDF$Diagnostic == "skade i venstre kne" ,]$Diagnostic = "Other specified injuries of left lower leg"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Patella femoral smertesyndrom",]$ICD10       = "M22"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Patella femoral smertesyndrom",]$Diagnostic  = "Disorder of patella"
                diseasesDBDF[diseasesDBDF$Diagnostic == "\"Slatters\" på høyre kne, noe med minisken på venstre kne",]$ICD10       = "M92.40"
                diseasesDBDF[diseasesDBDF$Diagnostic == "\"Slatters\" på høyre kne, noe med minisken på venstre kne",]$Diagnostic  = "Juvenile osteochondrosis of patella"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Slatters syndrom",]$ICD10       = "M92.40"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Slatters syndrom",]$Diagnostic  = "Juvenile osteochondrosis of patella"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Schlatters sykdom begge knær",]$ICD10       = "M92.40"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Schlatters sykdom begge knær",]$Diagnostic  = "Juvenile osteochondrosis of patella"
                diseasesDBDF[diseasesDBDF$Diagnostic == "beinhinnebetennelse ve,leggbein ca 1 års tid",]$ICD10       = "M86.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "beinhinnebetennelse ve,leggbein ca 1 års tid",]$Diagnostic  = "Osteomyelitis, unspecified"  
                diseasesDBDF[diseasesDBDF$Diagnostic == "kronisk beinhinnebetennelse i begge legger, skal opereres snart",]$ICD10       = "M86.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "kronisk beinhinnebetennelse i begge legger, skal opereres snart",]$Diagnostic  = "Osteomyelitis, unspecified"  
                # ---- Ok, since one is operated and the other is not, we keep only one strain
                diseasesDBDF[diseasesDBDF$Diagnostic == "Belastningssakde i begge knær, operert i hø. Kne den 8.10.2010.  venter på ve. Kne operasjon",]$ICD10       = "S86.91"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Belastningssakde i begge knær, operert i hø. Kne den 8.10.2010.  venter på ve. Kne operasjon",]$Diagnostic  = "Strain of unspecified muscle and tendon at lower leg level"
                # -------- Inflammation in both legs, so added both icd10s
                diseasesDBDF[diseasesDBDF$Diagnostic == "legghinnebetennelse i begge bein",]$ICD10       = "R22.42"
                diseasesDBDF[diseasesDBDF$Diagnostic == "legghinnebetennelse i begge bein",]$Diagnostic  = "Localized swelling, mass and lump, left lower limb"
                newRow = c(20066721, "Localized swelling, mass and lump, right lower limb", "R22.41", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                diseasesDBDF[diseasesDBDF$Diagnostic == "kronisk betennelse i venstre kne",]$ICD10       = "R22.42"
                diseasesDBDF[diseasesDBDF$Diagnostic == "kronisk betennelse i venstre kne",]$Diagnostic  = "Localized swelling, mass and lump, left lower limb"
        
                # ---- I want to register these two, but he doesn't specify what kind of problems. Whether is pain, lack of mobility, or whatever
                #      I'm going to say that at least, there is pain, so I can just register that.
                diseasesDBDF[diseasesDBDF$Diagnostic == "Rygg og kneproblemer pga idrettsskade" ,]$ICD10       = "M54.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Rygg og kneproblemer pga idrettsskade" ,]$Diagnostic  = "Dorsalgia, unspecified"
                newRow = c(20124413, "Other specified injuries of left lower leg", "S89.82", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                newRow = c(20124413, "Other specified injuries of right lower leg", "S89.81", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow

                # ---- Arms
                diseasesDBDF[diseasesDBDF$Diagnostic == "Senebetennelse begge håndledd",]$ICD10       = "M67.834"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Senebetennelse begge håndledd",]$Diagnostic  = "Other specified disorders of tendon, left wrist"
                newRow = c(20102611, "Other specified disorders of tendon, right wrist", "M67.833", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow

                # ---- Torso and abdomen
                diseasesDBDF[diseasesDBDF$Diagnostic == "Brokk, som gjør at lukkemuskelen til magen ikke fungerer adekvat.",]$ICD10          = "K45"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Brokk, som gjør at lukkemuskelen til magen ikke fungerer adekvat.",]$Diagnostic     = "Abdominal hernia"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Flere år med periodevise magesmerter uten at legene har funnet årsak",]$ICD10       = "R10.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Flere år med periodevise magesmerter uten at legene har funnet årsak",]$Diagnostic  = "Unspecified abdominal pain"
        
                # ------------------------------------------
                # Skin diseases
                # ------------------------------------------
                # ---- Add the dermatitis and the bone disease again later with the same ID
                diseasesDBDF[diseasesDBDF$Diagnostic == "pustolosids palmoplantaris ev dermatitt. Beinhinnebetennelse i begge legger i tre år",]$ICD10       = "L20.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "pustolosids palmoplantaris ev dermatitt. Beinhinnebetennelse i begge legger i tre år",]$Diagnostic  = "Atopic dermatitis, unspecified"
                newRow = c(20002913, "Osteomyelitis", "M86.9", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
                diseasesDBDF[diseasesDBDF$Diagnostic == "akne",]$ICD10            = "L70"
                diseasesDBDF[diseasesDBDF$Diagnostic == "akne",]$Diagnostic       = "Acne"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Elveblest",]$ICD10       = "L50.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Elveblest",]$Diagnostic  = "Urticaria, unspecified"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kuldeallergi. Får utslett. Også når vått og fuktig .",]$ICD10       = "L50.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kuldeallergi. Får utslett. Også når vått og fuktig .",]$Diagnostic  = "Urticaria due to cold and heat"
                diseasesDBDF[diseasesDBDF$Diagnostic == "vittiligo, hudsykdom",]$ICD10       = "L80"
                diseasesDBDF[diseasesDBDF$Diagnostic == "vittiligo, hudsykdom",]$Diagnostic  = "Vitiligio"

                diseasesDBDF[diseasesDBDF$Diagnostic == "Atopisk eksem",]$ICD10       = "L20.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Atopisk eksem",]$Diagnostic  = "Atopic dermatitis, unspecified"

                # ---- very complete description ¬¬
                diseasesDBDF[diseasesDBDF$Diagnostic == "hudsykdom",]$ICD10       = "L98.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "hudsykdom",]$Diagnostic  = "Disorder of the skin and subcutaneous tissue, unspecified"
                # ---- Dina's specific diagnosed this one as pruritus, so it stays in skin diseases
                diseasesDBDF[diseasesDBDF$Diagnostic == "irritasjon, kløe på øyne, diagnsostisert av lege",]$ICD10       = "L29.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "irritasjon, kløe på øyne, diagnsostisert av lege",]$Diagnostic  = "Pruritus, unspecified"
        
                # ------------------------------------------
                # Digestive track
                # ------------------------------------------
                diseasesDBDF[diseasesDBDF$Diagnostic == "Morbus krons",]$ICD10       = "K50.90"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Morbus krons",]$Diagnostic  = "Crohns disease"
                diseasesDBDF[diseasesDBDF$Diagnostic == "morbus crohn",]$ICD10       = "K50.90"
                diseasesDBDF[diseasesDBDF$Diagnostic == "morbus crohn",]$Diagnostic  = "Crohns disease"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Mb. Chron. Husker ikke når han fikk diagnosen. Var baby.",]$ICD10        = "K50.90"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Mb. Chron. Husker ikke når han fikk diagnosen. Var baby.",]$Diagnostic   = "Crohns disease"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Magesår 2 ganger. Siste påvist i forrige uke. Starter beh.",]$ICD10       = "K25"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Magesår 2 ganger. Siste påvist i forrige uke. Starter beh.",]$Diagnostic  = "Gastric ulcer"
        
                # ---- Not sure if this should even be register since is now solved.
                diseasesDBDF[diseasesDBDF$Diagnostic == "Født med øsofagusatresi (?). Ble behandlet i Tr.heim med blokking for 5 år siden.",]$ICD10       = "Q39.0"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Født med øsofagusatresi (?). Ble behandlet i Tr.heim med blokking for 5 år siden.",]$Diagnostic  = "Atresia of esophagus without fistula"        
                diseasesDBDF[diseasesDBDF$Diagnostic == "Glutenintoleranse.  Mangler enzymer. Oppdaget ved 15 års alderen",]$ICD10       = "K90.0"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Glutenintoleranse.  Mangler enzymer. Oppdaget ved 15 års alderen",]$Diagnostic  = "Celiac disease"
                # ---- As it is not specified the reason for pancreatitis, is listed as others
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kronisk pankreatitt",]$ICD10       = "K86.1"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kronisk pankreatitt",]$Diagnostic  = "Other chronic pancreatitis"
        
                # ------------------------------------------
                # Kidneys to genitals
                # ------------------------------------------
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kronisk urinveisinfeksjon.",]$ICD10       = "N39.0"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kronisk urinveisinfeksjon.",]$Diagnostic  = "Urinary tract infection, site not specified"
        
                # ------------------------------------------
                # General infectious diseases
                # ------------------------------------------
                diseasesDBDF[diseasesDBDF$Diagnostic == "kyssesyke ( påvist i sept 2010, syk i mai",]$ICD10       = "B27.90"
                diseasesDBDF[diseasesDBDF$Diagnostic == "kyssesyke ( påvist i sept 2010, syk i mai",]$Diagnostic  = "Infectious mononucleosis, unspecified without complication"

                # ------------------------------------------
                # Others
                # ------------------------------------------
                diseasesDBDF[diseasesDBDF$Diagnostic == "tinitus",]$ICD10       = "H93.1"
                diseasesDBDF[diseasesDBDF$Diagnostic == "tinitus",]$Diagnostic  = "Tinnitus"
                diseasesDBDF[diseasesDBDF$Diagnostic == "lettere hørselshemmet",]$ICD10       = "H90.5"
                diseasesDBDF[diseasesDBDF$Diagnostic == "lettere hørselshemmet",]$Diagnostic  = "Unspecified sensorineural hearing loss"        
                # ---- Person writing the questionnaires don't care about anonymization rules here T_T
                diseasesDBDF[diseasesDBDF$Diagnostic == "Thorbjørn har et syndrom exfragilsyndrom ( usikker hvordan det skrives)",]$ICD10       = "Q99.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Thorbjørn har et syndrom exfragilsyndrom ( usikker hvordan det skrives)",]$Diagnostic  = "Fragile X chromosome"        
                diseasesDBDF[diseasesDBDF$Diagnostic == "har i ett år hatt en tumor i høyre lår som skal opereres",]$ICD10       = "D49.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "har i ett år hatt en tumor i høyre lår som skal opereres",]$Diagnostic  = "Neoplasm of unspecified behavior of bone, soft tissue, and skin"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Håravfall ( husker ikke diagnose)",]$ICD10       = "L65.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Håravfall ( husker ikke diagnose)",]$Diagnostic  = "Nonscarring hair loss, unspecified"

                # ------------------------------------------
                # Other Autoimmune not listed before
                # ------------------------------------------
                diseasesDBDF[diseasesDBDF$Diagnostic == "Ideopotisk trombocytopenisk pulpura",]$ICD10       = "D69.3"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Ideopotisk trombocytopenisk pulpura",]$Diagnostic  = "Immune thrombocytopenic purpura"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Lupus",]$ICD10       = "L93"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Lupus",]$Diagnostic  = "Lupus erythematosus"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Autoimmun hepatitt",]$ICD10       = "K75.4"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Autoimmun hepatitt",]$Diagnostic  = "Autoimmune hepatitis"
        
                # ------------------------------------------
                # No clear diagnosis that are just deleted from the list of diseases
                # ------------------------------------------
                # ---- No diagnose, so deleted
                diseasesDBDF[diseasesDBDF$Diagnostic == "Mulig anstrengelsesutløst astma. Har medisin til utprøving nå. Ikke fått diagnose ennå.",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Mulig anstrengelsesutløst astma. Har medisin til utprøving nå. Ikke fått diagnose ennå.",]$Diagnostic  = ""        
                # ---- No diagnosis for migraine, so deleted
                diseasesDBDF[diseasesDBDF$Diagnostic == "Utredes for migrene",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Utredes for migrene",]$Diagnostic  = ""
                # ---- Taking meds but no disease given
                diseasesDBDF[diseasesDBDF$Diagnostic == "oppgir ikke kronisk sykdom men sier han ukentlig tar smertestillende mot hodepine, ibux eller pinex",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "oppgir ikke kronisk sykdom men sier han ukentlig tar smertestillende mot hodepine, ibux eller pinex",]$Diagnostic  = ""
                # ---- Frequent headaches, but no further description.
                diseasesDBDF[diseasesDBDF$Diagnostic == "Oppgir at han ofte har hodepine",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Oppgir at han ofte har hodepine",]$Diagnostic  = ""
                # ---- Knee problems, but unsure why. (Maybe is schlatter since a lot of other people also have it, but who knows)
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har alltid hatt problem med knærne,spes.hø. Ble verre rundt 12 års alderen. Løse leddbånd?",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har alltid hatt problem med knærne,spes.hø. Ble verre rundt 12 års alderen. Løse leddbånd?",]$Diagnostic  = ""
                # ---- Deleted as this is not chronic
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har hatt lavt stoffskifte etter virussykdom. Brukt medisin mnen bruker ikke dette i dag.",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har hatt lavt stoffskifte etter virussykdom. Brukt medisin mnen bruker ikke dette i dag.",]$Diagnostic  = ""
                # ---- Epilepsy now gone? deleted from the chronic disease list
                diseasesDBDF[diseasesDBDF$Diagnostic == "epilepsi som barn, ikke anfall på 5 år",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "epilepsi som barn, ikke anfall på 5 år",]$Diagnostic  = ""
                # ---- Didn't have asthma since 6 years old, so it get deleted (later we clean the "" cases)
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har ikke astma lenger siden 6 år gammel.",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har ikke astma lenger siden 6 år gammel.",]$Diagnostic  = ""
                # ---- Same, no asthma anymore
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har hatt astma men har det ikke lengre",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Har hatt astma men har det ikke lengre",]$Diagnostic  = ""
                # ---- the disease is gone now, so deleted
                diseasesDBDF[diseasesDBDF$Diagnostic == "kyssesyke i over 2 år, gått over nå",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "kyssesyke i over 2 år, gått over nå",]$Diagnostic  = ""
                # ---- Nothing to report, so deleted
                diseasesDBDF[diseasesDBDF$Diagnostic == "Hjerteoperert som nyfødt. Går til kontroller en gang per år",]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "Hjerteoperert som nyfødt. Går til kontroller en gang per år",]$Diagnostic  = ""
                # ---- Don't we all T_T , anyway, not chronic, and the interview was not in winter, so deleted
                diseasesDBDF[diseasesDBDF$Diagnostic == "søvnproblem i en vinterperiode" ,]$ICD10       = ""
                diseasesDBDF[diseasesDBDF$Diagnostic == "søvnproblem i en vinterperiode" ,]$Diagnostic  = ""

                diseasesDBDF[diseasesDBDF$Diagnostic == "regnbuehinnebetendelse  for 7 år siden. og leddgikt",]$ICD10       = "H20"
                diseasesDBDF[diseasesDBDF$Diagnostic == "regnbuehinnebetendelse  for 7 år siden. og leddgikt",]$Diagnostic  = "Iridocyclitis"
                newRow = c(20112006, "Juvenile arthritis", "M08", NA, NA, NA)
                diseasesDBDF[nrow(diseasesDBDF)+1,] = newRow
        
                # ---- Can't find the specific area of a cyst unless it is a neoplasm in the icd10 codes.
                diseasesDBDF[diseasesDBDF$Diagnostic == "cyste hø side i hodet, uspesifisert diagnose iflg pas",]$ICD10       = "L72.3"
                diseasesDBDF[diseasesDBDF$Diagnostic == "cyste hø side i hodet, uspesifisert diagnose iflg pas",]$Diagnostic  = "Sebaceous cyst"
        
                diseasesDBDF[diseasesDBDF$Diagnostic == "Problemene kommer ca midt i måltidet. Smerter på venstre halvdel av magen, opp mot navelen.",]$ICD10       = "R10.9"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Problemene kommer ca midt i måltidet. Smerter på venstre halvdel av magen, opp mot navelen.",]$Diagnostic  = "Unspecified abdominal pain"

                diseasesDBDF[diseasesDBDF$Diagnostic == "Er deperimert fra 18 års alder" ,]$ICD10       = "F33.8"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Er deperimert fra 18 års alder" ,]$Diagnostic  = "Other recurrent depressive disorders"
        
                diseasesDBDF[diseasesDBDF$Diagnostic == "en del hodepine",]$ICD10       = "R51"
                diseasesDBDF[diseasesDBDF$Diagnostic == "en del hodepine",]$Diagnostic  = "Headache"
        
                diseasesDBDF[diseasesDBDF$Diagnostic == "PCOS",]$ICD10       = "E28.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "PCOS",]$Diagnostic  = "Polycystic ovarian syndrome"
        
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kronisk smertesyndrom, type 2. (nyreproblematikk)",]$ICD10       = "N18.2"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kronisk smertesyndrom, type 2. (nyreproblematikk)",]$Diagnostic  = "Chronic kidney disease, stage 2 (mild)"
                
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kreft i skjoldbruskkjertel. Op 9.mars 2010.",]$ICD10       = "C73"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Kreft i skjoldbruskkjertel. Op 9.mars 2010.",]$Diagnostic  = "Malignant neoplasm of thyroid gland"
        
                diseasesDBDF[diseasesDBDF$Diagnostic == "Dårlig funksjon av lukkemuskel ned til magen.",]$ICD10       = "N39"
                diseasesDBDF[diseasesDBDF$Diagnostic == "Dårlig funksjon av lukkemuskel ned til magen.",]$Diagnostic  = "Other disorders of urinary system"
...
# -----------------------------------------------------------------------------
{
    ...
    # ---- Convert numbers into categories
    {
        ...
        # enumerations. Or ontology anotations.
        ...
        # Basics
        {
            ...
            basicTable$GeneralHealth = mapvalues( basicTable$GeneralHealth,
                                                  from = c( 1,          2,     3,                      4,      5,           " ",        "  ",        NA),
                                                  to   = c("Very bad", "Bad", "Neither good nor bad", "Good", "Excellent",  "Unknown",  "Unknown",  "Unknown"))
            ...
            for (i in 1:originalTotalRows){
                ...
                if( total1s > 0 ){
                    ...
                    if(Mother_Work_Fulltime1 == TRUE){
                        sociologyTable$Mother_WorkTime = "Full time"
                    }
                    else{
                        if(Mother_Work_Parttime1 == TRUE){
                            sociologyTable$Mother_WorkTime = "Part time"    
                        }
                        else{
                            if(Mother_Work_Unemployed1 == TRUE){
                                sociologyTable$Mother_WorkTime = "Unemployed"        
                            }
                            else{
                                if(Mother_Work_Pensioned1 == TRUE){
                                    sociologyTable$Mother_WorkTime = "Pensioned"        
                                }
                                else{
                                    if(Mother_Work_Deceased1 == TRUE){
                                        sociologyTable$Mother_WorkTime = "Deceased"        
                                    }
                                    else{
                                        if(Mother_Work_DontKnown1 == TRUE){
                                            sociologyTable$Mother_WorkTime = "Don't know" 
                                        }
                                        else{
                                            if(Mother_Work_Other1 == TRUE){
                                                sociologyTable$Mother_WorkTime = "Other" 
                                            }
            ...
            for (i in 1:originalTotalRows){
                ...
                if( total1s > 0 ){
                    ...
                    if(Father_Work_Fulltime1 == TRUE){
                        sociologyTable$Father_WorkTime = "Full time"
                    }
                    else{
                        if(Father_Work_Parttime1 == TRUE){
                            sociologyTable$Father_WorkTime = "Part time"    
                        }
                        else{
                            if(Father_Work_Unemployed1 == TRUE){
                                sociologyTable$Father_WorkTime = "Unemployed"        
                            }
                            else{
                                if(Father_Work_Pensioned1 == TRUE){
                                    sociologyTable$Father_WorkTime = "Pensioned"        
                                }
                                else{
                                    if(Father_Work_Deceased1 == TRUE){
                                        sociologyTable$Father_WorkTime = "Deceased"        
                                    }
                                    else{
                                        if(Father_Work_DontKnown1 == TRUE){
                                            sociologyTable$Father_WorkTime = "Don't know" 
                                        }
                                        else{
                                            if(Father_Work_Other1 == TRUE){
                                                sociologyTable$Father_WorkTime = "Other" 
                                            }
                                            # Default is Didn't answer, so no need to go deeper    
                                        }
                                    }
            ...
            for (i in 1:originalTotalRows){
                ...
                if(!is.na(sociologyTable$EthnicityOther[i])){
                    if(sociologyTable$EthnicityOther[i] == 1){
                        ...
                        if(EthnicComment == "")                                                EthnicityOther = ""
                        if(EthnicComment == "afgansk")                                         EthnicityOther = "Afghan"                            
                        if(EthnicComment == "afghans")                                         EthnicityOther = "Afghan"
                        if(EthnicComment == "afghansk")                                        EthnicityOther = "Afghan"                                               
                        if(EthnicComment == "africansk")                                       EthnicityOther = "African"
                        if(EthnicComment == "afrikansk")                                       EthnicityOther = "African"                            
                        if(EthnicComment == "annet, spesifiser her")                           EthnicityOther = ""
                        if(EthnicComment == "awesome")                                         EthnicityOther = ""                            
                        if(EthnicComment == "belgisk")                                         EthnicityOther = "Belgian"
                        if(EthnicComment == "bodd over alt i verden")                          EthnicityOther = "Other"
                        if(EthnicComment == "brasiliansk")                                     EthnicityOther = "Brasilian"
                        if(EthnicComment == "bulgarsk")                                        EthnicityOther = "Bulgarian" 
                        if(EthnicComment == "canadisk")                                        EthnicityOther = "Canadian"
                        if(EthnicComment == "colombiansk")                                     EthnicityOther = "Colombian"                                                    
                        if(EthnicComment == "dansk")                                           EthnicityOther = "Danish"
                        if(EthnicComment == "eritrea")                                         EthnicityOther = "Eritrean"
                        if(EthnicComment == "eritreisk")                                       EthnicityOther = "Eritrean"
                        if(EthnicComment == "fjellfinn")                                       EthnicityOther = "Sami"     # This person doesn't consider himself Sami, but I'm changing it to Sami anyway since is more politically correct
                        if(EthnicComment == "fordi jeg ikke er etniske norsk eller de andre.") EthnicityOther = "Other"
                        if(EthnicComment == "gambisk")                                         EthnicityOther = "Gambian"
                        if(EthnicComment == "halvt afrikansk. faren min er fra afrika.")       EthnicityOther = "African"
                        if(EthnicComment == "halvt dansk")                                     EthnicityOther = "Danish"                            
                        if(EthnicComment == "halvt italiensk.")                                EthnicityOther = "Italian"
                        if(EthnicComment == "halvt norsk og halvt ghanesisk")                  EthnicityOther = "Norwegian-Ghanaian" # This person say half and half, but Norwegian = FALSE, so I correct it here
                        if(EthnicComment == "halvt tysk")                                      EthnicityOther = "German"
                        if(EthnicComment == "islandsk")                                        EthnicityOther = "Icelandic" 
                        if(EthnicComment == "italiensk")                                       EthnicityOther = "Italian"
                        if(EthnicComment == "kinesisk")                                        EthnicityOther = "Chinese"
                        if(EthnicComment == "min mor er finsk og min far en norsk.")           EthnicityOther = "Finnish"
                        if(EthnicComment == "nederlandsk")                                     EthnicityOther = "Dutch"                        
                        if(EthnicComment == "nederlandsk og svensk")                           EthnicityOther = "Dutch-Swedish"
                        if(EthnicComment == "nord-norsk lenangsværing")                        EthnicityOther = "Norwegian"
                        if(EthnicComment == "nord-norsk!")                                     EthnicityOther = "Norwegian"
                        if(EthnicComment == "nordlænning")                                     EthnicityOther = "" # This person already answered Norwegian
                        if(EthnicComment == "nordnorsk")                                       EthnicityOther = "" # Same
                        if(EthnicComment == "norsk og filipinsk")                              EthnicityOther = "Norwegian-Philipine"
                        if(EthnicComment == "norsk somalisk")                                  EthnicityOther = "Norwegian-Somali"
                        if(EthnicComment == "norsk/kvensk")                                    EthnicityOther = "Norwegian-Kven"                                                
                        if(EthnicComment == "norsk/somalisk")                                  EthnicityOther = "Somalian" # This person already answered Norwegian
                        if(EthnicComment == "norsk/svensk")                                    EthnicityOther = "Norwegian-Swedish"                            
                        if(EthnicComment == "norsktamiler")                                    EthnicityOther = "Other"
                        if(EthnicComment == "palestinsk")                                      EthnicityOther = "Palestinian"
                        if(EthnicComment == "pappa kommer fra belgia")                         EthnicityOther = "Belgian"
                        if(EthnicComment == "polsk")                                           EthnicityOther = "Polish"
                        if(EthnicComment == "portugisisk")                                     EthnicityOther = "Portuguese"
                        if(EthnicComment == "russisk")                                         EthnicityOther = "Russian"
                        if(EthnicComment == "russisk/norsk")                                   EthnicityOther = "Norwegian-Russian"                            
                        if(EthnicComment == "somalisk")                                        EthnicityOther = "Somalian"
                        if(EthnicComment == "spansk")                                          EthnicityOther = "Spanish" 
                        if(EthnicComment == "svensk")                                          EthnicityOther = "Swedish"
                        if(EthnicComment == "tamil")                                           EthnicityOther = "Tamil"
                        if(EthnicComment == "tamilsk")                                         EthnicityOther = "Tamil" 
                        if(EthnicComment == "thai")                                            EthnicityOther = "Thai"                            
                        if(EthnicComment == "tyrkisk")                                         EthnicityOther = "Turquish"
                        if(EthnicComment == "tysk")                                            EthnicityOther = "German"
                ...
                # Cases where we have more than one
                    ...
                    if(EthnicityNorwegian != "") paste0(finalEthnicity, EthnicityNorwegian, "-")
                    if(EthnicitySami      != "") paste0(finalEthnicity, EthnicitySami,      "-")
                    if(EthnicityKven      != "") paste0(finalEthnicity, EthnicityKven,      "-")
                    if(EthnicityOther     != "") paste0(finalEthnicity, EthnicityOther,     "-")
        ...
        # Puberty men    
        {
            ...
            pubertyMenTable$ChangeHeight = mapvalues( pubertyMenTable$ChangeHeight,
                                           from = c(  1,                 2,                3,                      4,           NA),
                                           to   = c( "Haven't Started", "Barely Started", "Definitely Underway",  "Completed", "Didn't Answered"))
        ...
        # Puberty women
        {
            ...
            pubertyWomenTable$Breats         = mapvalues( pubertyWomenTable$Breats,
                                               from = c( 1,     0,    NA),
                                               to   = c("Yes", "No", "Unknown"))            
        ...
        # Menstruation
        {
            ...
            menstruationTable$MenstruationRegular = mapvalues( menstruationTable$MenstruationRegular,
                                                      from = c( "1",              "2",               "3",         "4",        NA),
                                                      to   = c( "Always regular", "Usually regular", "Irregular", "Unknown",  "Unknown"))
        ...
        # Drugs
        {
            ...
            drugsTable$SmokePerWeek  = mapvalues( drugsTable$SmokePerWeek,
                                           from = c( "1",      "2",      "3",      "4",       "5",             NA),
                                           to   = c( "0 to 1", "2 to 3", "4 to 6", "7 to 10", "More than 10", "Unknown"))        
            ...
            drugsTable$AlcoholUnits  = mapvalues( drugsTable$AlcoholUnits,
                                                  from = c( "1",      "2",      "3",      "4",      "5",           NA),
                                                  to   = c( "1 to 2", "3 to 4", "5 to 6", "7 to 9", "10 or More", "Unknown"))        
            
            drugsTable$Alcohol6Units  = mapvalues( drugsTable$Alcohol6Units,
                                                   from = c( "1",      "2",                 "3",       "4",      "5",      NA),
                                                   to   = c( "Never",  "Less than monthly", "Monthly", "Weekly", "Daily", "Unknown"))        
        ...
        # Sports
        {
            ...
            sportsTable$SportsFrequency     = mapvalues( sportsTable$SportsFrequency,
                                                         ...
                                                         to   = c( "Never", "Less than daily", "1 per week", "2-3 per week", "4-6 per week", "Almost every day", "Uknown"))

            sportsTable$SportHours          = mapvalues( sportsTable$SportHours,
                                                         from = c( "1",     "2",                 "3",              "4",            "5",            "6",               NA),
                                                         to   = c( "None", "About half an hour", "1 to 1.5 hours", "2 to 3 hours", "4 to 6 hours", "7 hours or more", "Uknown"))            

            sportsTable$SportsIntensity     = mapvalues( sportsTable$SportsIntensity,
                                                         from = c( "1",               "2",          "3",          "4",         "5",              NA),
                                                         to   = c( "Not hard at all", "A bit hard", "Quite hard", "Very hard", "Extremely hard", "Uknown"))                        
        
            sportsTable$SummerTransport     = mapvalues( sportsTable$SummerTransport,
                                                         from = c( "1",               "2",      "3",       "4",       NA),
                                                         to   = c( "By car or moped", "By bus", "By bike", "On foot", "Uknown"))
            
            sportsTable$SummerTime          = mapvalues( sportsTable$SummerTime,
                                                         from = c( "1",          "2",            "3",             "4",     "5",           NA),
                                                         to   = c( "<5 minutes", "6-15 minutes", "16-30 minutes", "31-60", ">60 minutes", "Uknown"))                                
        ...
        # Hygiene
        {
            ...
            hygieneTable$ShowerBathFrequency  = mapvalues( hygieneTable$ShowerBathFrequency,
                                                from = c(  1,                       2,             3,                  4,                 5,            6,               7,                       NA),
                                                to   = c( "Less than once a week", "Once a week", "2-3 times a week", "4-6 times a week","Once a day", "2 times a day", "3 or more times a day", "Unknown"))
                     
                        
            hygieneTable$HandwashFrequency    = mapvalues(  hygieneTable$HandwashFrequency,
                                                from = c(  1,         2,           3,           4,             5,             6,                    NA),
                                                to   = c( "0 times", "1-2 times", "3-5 times", "6-10 times",  "11-20 times", "More than 20 times", "Unknown"))
            
            hygieneTable$BodyLotionFrequency  = mapvalues(  hygieneTable$BodyLotionFrequency,
                                                from = c(  1,                       2,             3,                  4,                   5,            6,                       NA),
                                                to   = c( "Less than once a week", "Once a week", "2-3 times a week", "4-6 times a week",  "Once a day", "2 or more times a day", "Unknown"))

                        
            hygieneTable$SkinSunbathing       = mapvalues( hygieneTable$SkinSunbathing,
                                                from = c(  1,                         2,                                    3,                                   4 ,                        NA),
                                                to   = c( "Always red, never brown", "Almost always red, sometimes brown", "Almost always brown, sometimes red","Always brown, never red", "Unknown"))
        ...
        # Diet ( this is a nightmare T_T )
        {
            ...
            dietTable$BreakfastFrequency     = mapvalues( dietTable$BreakfastFrequency,
                                                         from = c( "1",            "2",                  "3",                  "4",          NA),
                                                         to   = c( "Rarely/Never", "1-3 times per week", "4-6 times per week", "Every day",  "Uknown"))
            ...
            dietTable$FruitsFrequency     = mapvalues( dietTable$FruitsFrequency,
                                                         from = c( "1",            "2",                   "3",                  "4",                  "5",                 "6",                 "7",                       NA),
                                                         to   = c( "Rarely/Never", "1-3 times per month", "1-3 times per week", "4-6 times per week", "1-2 times per day", "3-4 times per day", "5 times or more per day", "Uknown"))
            ...
            dietTable$DairyFrequency     = mapvalues( dietTable$DairyFrequency,
                                                         from = c( "1",            "2",                    "3",               "4",                   "5",                         NA),
                                                         to   = c( "Rarely/Never", "1-6 glasses per week", "1 glass per day", "2-3 glasses per day", "4 or more glasses per day", "Uknown"))
            ...
            dietTable$CheeseFrequency     = mapvalues( dietTable$CheeseFrequency,
                                                         from = c( "1",            "2",                   "3",                  "4",                  "5",         NA),
                                                         to   = c( "Rarely/Never", "1-3 times per month", "1-3 times per week", "4-6 times per week", "Every day", "Uknown"))
            ...
            dietTable$SeagullEggsFrequency     = mapvalues( dietTable$SeagullEggsFrequency,
                                                         from = c( "1",            "2",                  "3",                  "4",                  "5",                         NA),
                                                         to   = c( "Rarely/Never", "1-3 times per year", "4-5 times per year", "6-9 times per year", "10 or more times per year", "Uknown"))            
            ...
            dietTable$FishOilFrequency  = mapvalues( dietTable$FishOilFrequency,
                                                    from = c( "1",    "2",          "3",          NA),
                                                    to   = c( "No",   "Sometimes",  "Yes, daily", "Unknown"))
        ...
        # Sleeping
        {
            ...
            sleepTable$SleepingPills   = mapvalues( sleepTable$SleepingPills,
                                                    from = c( "1",          "2",                                  "3",                         "4",     NA),
                                                    to   = c( "Not used",   "Less frequently than every week",    "Every week, but not daily", "Daily", "Unknown"))            
        ...
        # Friendship
        {
            ...
            frienshipTable$YesterdaySMS = mapvalues( frienshipTable$YesterdaySMS,
                                          from = c(  1,      2,        3,         4,          5,          6,     NA),
                                          to   = c( "None", "1 to 5", "6 to 10", "11 to 20", "21 to 50", ">50", "Unknown"))
...
# There is still one thing missing, which is the levels. Levels are use to sort
# variables such as:
#
#       Never, rarely, sometimes, frequently, always
#
# So when you plot or summarize then, they keep that order, and not sorted
# alphabetically or whatever.
#
# The other use is to keep consistency in the color palette. For example:
#
#       Man , Woman
#
# Although these variables don't have an intrinsecal order, sometimes a Man
# comes first, and some other times a Woman comes first, in whatever table you
# are analyzing. So you need to keep the order so Man goes to color 1, and 
# Woman goes to color 2 always.
#
# This is not done here, since this is not keep in the CSV files that this
# script is going to generate.
#
# So this need to be done by the script that loads the data, in whatever 
...
writeTableLATEX(dataCleaningLogDF, LOGS_FOLDER, 
                ...
                overrideTableName = "SummaryLost", widthProportion = 0.9, heightProportion = 0.05)
