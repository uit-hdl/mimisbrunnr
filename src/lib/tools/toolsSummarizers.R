...
summarizeCategorical <- function(tableBase, countingIndex, sorted = "top", crop = 0, roundMe = 0){
  ...
  myModalities = sort(unique(tableBase[,countingIndex]))
  if(countingVariableType == "factor"){
    myModalities    = levels(tableBase[,countingIndex])    
  }
  ...
  for(i in 1:totalModalities){
    ...
    summaryDF$Count[i]    = sum( tableBase[,countingIndex] == currentModality )
